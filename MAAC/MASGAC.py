from .model import Critic, ActorMAAC, PerceptGNN, SGActor, SGCritic
import torch as th
...
class MAFO_GAC: # Multi-Agent fully observable actorcritic
    def __init__(self, n_agents, observation_shape, dim_act, batch_size,
        ...
        self.actors = [SGActor(observation_shape, dim_act) for i in range(n_agents)]

        # self.critics_gnn = PerceptGNN((*observation_shape[:2],n_agents*observation_shape[2]))
        # self.critics_gnn = PerceptGNN((*observation_shape[:2],n_agents*observation_shape[2]))
        self.critics = SGCritic(n_agents, (*observation_shape[:2],n_agents*observation_shape[2]),
                               dim_act)
