...
class GActor(nn.Module):
    def __init__(self,observation_shape, dim_action):
        ...
        self.gnn3 = DenseGATConv(32*4, 16, heads=4, dropout=0)
        self.gnn4 = DenseGATConv(16*4, 16, heads=4, dropout=0)
        self.dim_observation = 16*4 * np.prod(observation_shape[:-1])
        self.FC1 = nn.Linear(16*4 * np.prod(observation_shape[:-1]), 64)
        self.FC2 = nn.Linear(64, dim_action)
...
class SGActor(nn.Module):
    def __init__(self,observation_shape, dim_action):
        super(SGActor, self).__init__()
        self.gnn1 = DenseGATConv(observation_shape[-1], 32, heads=4, dropout=0)
        self.gnn2 = DenseGATConv(32*4, 16, heads=4, dropout=0)
        self.dim_observation = 16*4 * np.prod(observation_shape[:-1])
        self.FC1 = nn.Linear(16*4 * np.prod(observation_shape[:-1]), dim_action)
